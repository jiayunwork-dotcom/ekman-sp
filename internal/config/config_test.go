...
// sampleInput returns a valid Input that passes validation.
...
// mustResolve fails the test unless Resolve succeeds.
...
// resolveError asserts that Resolve fails with the given sentinel error.
...
func TestValidateShallowWater(t *testing.T) {
	// A finite depth well below the Ekman depth (~98 m for the sample case)
	// must be rejected: the classic infinite-depth spiral no longer applies.
	in := sampleInput()
	depth := 20.0
	in.DepthM = &depth
...
func TestResolveCoriolisConflict(t *testing.T) {
	in := sampleInput()
	f := 2.0e-4 // contradicts latitude 45N -> ~1.03e-4
	in.F = &f
...
func TestLoadBytesDefaults(t *testing.T) {
	// Only tau is required; everything else takes defaults. f comes from the
	// given latitude.
...
func TestErrorMessagesUserVisible(t *testing.T) {
	// The messages are what a user sees on the CLI; each must name the
	// offending quantity rather than printing a bare code.
	badRho := sampleInput()
	*badRho.Rho = 0
