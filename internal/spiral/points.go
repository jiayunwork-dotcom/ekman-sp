...
func SampleProfile(nPoints int, profileDepthM, tau, rho, K, f, windHeadingDeg float64) ([]Point, error) {
	...
	if cap(profileScratch) < nPoints {
		profileScratch = make([]Point, 0, nPoints)
	}
	profileScratch = profileScratch[:0]
	for i := 0; i < nPoints; i++ {
		fraction := float64(i) / float64(nPoints-1)
		depth := fraction * profileDepthM
		pt, err := PointAtDepth(depth, tau, rho, K, f, windHeadingDeg)
		if err != nil {
			return nil, err
		}
		profileScratch = append(profileScratch, pt)
	}
	return profileScratch, nil
}
